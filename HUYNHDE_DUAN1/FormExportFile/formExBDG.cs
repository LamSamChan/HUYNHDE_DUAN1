...
{
    public partial class formExBDG : Form
    {
        #region Border Forms
        ...
        public formExBDG(formGDBDG _GDBDG)
        {
            InitializeComponent();
        ...
        private void excel_Click(object sender, EventArgs e)
        {
            ...
            data = GDBDG.GridBDG;
            string title = "DỮ LIỆU GIAO DICH BIẾN ĐỘNG GIÁ";
            ...
            if (data.Rows.Count > 0)
            {
                ...
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (!fileError)
                    {
                        ...
                        {
                            throw; //f.showMessage("Thông báo", $"{ex.Message}", "icon_error", "Đóng");
                        }
                    }
                }
            }
        ...
        private void pdf_Click(object sender, EventArgs e)
        {
            ...
            string title = "DỮ LIỆU GIAO DỊCH BIẾN ĐỘNG GIÁ ";
            if (data.Rows.Count > 0)
            {
                SaveFileDialog sfd = new SaveFileDialog();
                sfd.Filter = "PDF (*.pdf)|*.pdf";
                sfd.FileName = "DuLieuBDG.pdf";
                bool fileError = false;
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    if (File.Exists(sfd.FileName))
